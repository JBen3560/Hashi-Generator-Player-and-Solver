...
// Function to check if an node has no neighbors
bool BoardMaker::noNeighbors(int r, int c, GridPosition** board){
    if(r > 0 && board[r-1][c].getNode() != '.') return false;
    if(r < 9 && board[r+1][c].getNode() != '.') return false;
    if(c > 0 && board[r][c-1].getNode() != '.') return false;
    if(c < 9 && board[r][c+1].getNode() != '.') return false;
    return true;
}

// Function to check if a new node will be valid
bool BoardMaker::validNewNode(int dist, char dir, int r, int c, GridPosition** board){
    ...
    for(int i = 1; i <= dist; i++){
        if(dir == 'n' && board[r-i][c].getNode() != '.') return false;
        if(dir == 'e' && board[r][c+i].getNode() != '.') return false;
        if(dir == 's' && board[r+i][c].getNode() != '.') return false;
        if(dir == 'w' && board[r][c-i].getNode() != '.') return false;
    }
    return true;
}
...
void BoardMaker::setUpBoard(int r, int c, GridPosition** board){
    ...
    for(int i = 0; i < 4; i++){
        ...
        if(validNewNode(dist, dirs[i], r, c, board)){
            // Determine whether the branch is one or two chains
            char branchType;
            int doubleBranch = randomNum(0,1);
            if(dirs[i] == 'n' || dirs[i] == 's') branchType = (doubleBranch) ? 'H' : '|';
            if(dirs[i] == 'e' || dirs[i] == 'w') branchType = (doubleBranch) ? '=' : '-';
            
            // Place chains in branch
            for(int j = 1; j < dist; j++){
                if(dirs[i] == 'n') board[r-j][c].setNode(branchType);
                if(dirs[i] == 'e') board[r][c+j].setNode(branchType);
                if(dirs[i] == 's') board[r+j][c].setNode(branchType);
                if(dirs[i] == 'w') board[r][c-j].setNode(branchType);
            }
